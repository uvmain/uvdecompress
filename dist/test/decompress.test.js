...
async function getBufferFromFixtureFile(filePath) {
...
describe('decompress', () => {
    ...
    it('extract multiple files from tar.gz', async () => {
        ...
        assert.strictEqual(files.find(f => f.path === 'test.txt')?.data.toString(), 'this is a test\n');
        assert.strictEqual(files.find(f => f.path === 'test2.txt')?.data.toString(), 'this is a test\n');
    });
